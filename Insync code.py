...
def make_student(name, age, grade, health,
    ...
    Keeps record-creation logic in one place.
    """
    global next_id
    ...
    student = {
        ...
        "contacts"  : [                    # list — supports multiple contacts
            {
                "name"        : contact_name,
...
def delete_student(student_id):
    ...
    # List comprehension — ITERATIVE
    students = [s for s in students if s["id"] != student_id]

    if len(students) < original_count:         # CONDITIONAL: was anything removed?
        save_to_file()
        return True
    return False
...
def validate_fields(fields_dict):
    """
    FUNCTION + ITERATIVE + CONDITIONAL:
    Checks that every field in the dict has a non-empty value.
...
def make_entry(parent, label_text, variable, hide=False):
    ...
    tk.Label(parent, text=label_text,
             ...
             font=FONT_LABEL).pack(pady=(5, 0))
    tk.Entry(parent, textvariable=variable,
             show="*" if hide else "",
             bg=ENTRY_BG, fg=FG_COLOR,
             insertbackground="white",
             justify="center",
             font=FONT_LABEL).pack(pady=4)


def make_button(parent, text, command, color=BTN_COLOR, width=22):
    """FUNCTION: add a styled button to a parent widget."""
...
def make_table(parent, columns, col_width=130):
    ...
    Uses a for loop to configure each column.
    """
...
def student_form():
    ...
    tk.Label(frame, text="─── Emergency Contact ───",
    ...
    make_button(frame, "Submit", submit, color=ACCENT_COLOR)
...
def staff_login():
    ...
    make_button(frame, "Login", login, color=ACCENT_COLOR)
    make_button(frame, "Back",  main_menu)
...
def staff_menu():
    ...
    tk.Label(frame, text=f"Total Records on File: {len(students)}",
...
def view_health():
    ...
    if not students:                           # CONDITIONAL
        tk.Label(root, text="No records found.",
                 bg=BG_COLOR, fg="#888888").pack(pady=20)
    else:
        columns = ("ID", "Name", "Age", "Grade",
                   "Health Condition", "Registered")
...
def search_student():
    ...
    tk.Label(search_frame, text="Enter Name:",
             bg=BG_COLOR, fg=FG_COLOR,
             font=FONT_LABEL).pack(side="left", padx=6)
    tk.Entry(search_frame, textvariable=search_var,
             bg=ENTRY_BG, fg=FG_COLOR,
             insertbackground="white",
             font=FONT_LABEL, width=24).pack(side="left", padx=6)
    ...
    def perform_search():
        ...
        if not results:                        # CONDITIONAL
            tk.Label(result_frame,
                     ...
                     bg=BG_COLOR, fg="#888888").pack(pady=10)
            return
...
def delete_record():
    ...
    tk.Label(frame, text="Enter Student ID to delete:",
             bg=BG_COLOR, fg=FG_COLOR,
             font=FONT_LABEL).pack(pady=(10, 4))

    id_var      = tk.StringVar()
    preview_var = tk.StringVar(value="")

    tk.Entry(frame, textvariable=id_var,
             bg=ENTRY_BG, fg=FG_COLOR,
             insertbackground="white",
             justify="center",
             font=FONT_LABEL).pack(pady=6)
    ...
    make_button(frame, "Confirm Delete", confirm_delete, color="#7f1d1d")
...
def main_menu():
    ...
    tk.Label(frame,
             text="Student Health & Emergency Record System",
             bg=BG_COLOR, fg="#888888",
    ...
    count = len(students)
    tk.Label(frame,
             text=f"{count} record(s) currently on file",
             bg=BG_COLOR, fg="#555555",
             font=FONT_SMALL).pack(pady=(16, 0))
